...
def eval_lrs2(engine, dataset):
    ...
    wer_score = wer(reference=label_list, hypothesis=output_list)
...
def eval_avcocktail(engine, video_dataset, label_dataset, set_name=None):
    ...
    for chunk_type in ['asd_chunk', 'fixed_chunk', 'gold_chunk']:
        ...
        for sample in tqdm(video_dataset[chunk_type], desc=f"Processing {chunk_type} {set_name}", total=len(video_dataset[chunk_type])):
...
def main():
    ...
    parser.add_argument(
        ...
        # required=True, 
        default='*',
        choices=['test', 'test_snr_n5_interferer_1', 'test_snr_n5_interferer_2', 'test_snr_0_interferer_1', 'test_snr_0_interferer_2', 'test_snr_5_interferer_1', 'test_snr_5_interferer_2', 'test_snr_10_interferer_1', 'test_snr_10_interferer_2'] + [f'video_{i}' for i in range(0, 51)],
        help='Path to folder containing session data (supports glob patterns with *)'
    )
    ...
    if args.dataset_name == 'lrs2':
        if args.set_id not in ['test', 'test_snr_n5_interferer_1', 'test_snr_n5_interferer_2', 'test_snr_0_interferer_1', 'test_snr_0_interferer_2', 'test_snr_5_interferer_1', 'test_snr_5_interferer_2', 'test_snr_10_interferer_1', 'test_snr_10_interferer_2', '*']:
            raise ValueError(f"Invalid set_id: {args.set_id} for LRS2 dataset. Only test and test_snr_n5_interferer_1, test_snr_n5_interferer_2, test_snr_0_interferer_1, test_snr_0_interferer_2, test_snr_5_interferer_1, test_snr_5_interferer_2, test_snr_10_interferer_1, test_snr_10_interferer_2, * are supported")
        if args.set_id == '*':
            list_wer_scores = []
            for set_id in ['test', 'test_snr_n5_interferer_1', 'test_snr_n5_interferer_2', 'test_snr_0_interferer_1', 'test_snr_0_interferer_2', 'test_snr_5_interferer_1', 'test_snr_5_interferer_2', 'test_snr_10_interferer_1', 'test_snr_10_interferer_2']:
                print(f"Inferring {args.dataset_name}/{set_id} sessions using {args.model_type} model")
                dataset = datasets.load_dataset("nguyenvulebinh/AVYT", args.dataset_name, cache_dir='./data-bin/cache', streaming=True)[set_id]
                wer_score = eval_lrs2(engine, dataset)
            ...
            dataset = datasets.load_dataset("nguyenvulebinh/AVYT", args.dataset_name, cache_dir='./data-bin/cache', streaming=True)[args.set_id]
            wer_score = eval_lrs2(engine, dataset)
            print(f"WER: {wer_score}")
    elif args.dataset_name == 'AVCocktail':
        if args.set_id not in [f'video_{i}' for i in range(0, 51)] + ['*']:
            raise ValueError(f"Invalid set_id: {args.set_id} for AVCocktail dataset. Only video_i for i in range(0, 51) and * are supported")
        if args.set_id == '*':
            list_wer_scores = {}
            for set_id in [f'video_{i}' for i in range(0, 51)]:
                ...
                label_dataset = datasets.load_dataset("nguyenvulebinh/AVCocktail", 'labels', cache_dir='./data-bin/cache')[set_id]
            ...
            print(f"Inferring {args.dataset_name}/{args.set_id} sessions using {args.model_type} model")
            video_dataset = datasets.load_dataset("nguyenvulebinh/AVCocktail", args.set_id, cache_dir='./data-bin/cache')
            label_dataset = datasets.load_dataset("nguyenvulebinh/AVCocktail", 'labels', cache_dir='./data-bin/cache')[args.set_id]
