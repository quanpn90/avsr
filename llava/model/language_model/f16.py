...
class F16Config(Qwen2Config):
    model_type = "f16"


class F16Model(LlavaMetaModel, Qwen2Model):
    config_class = F16Config

    def __init__(self, config: Qwen2Config):
        super(F16Model, self).__init__(config)
        ...
        self.frame_repeat_num = config.model_args.get("frame_repeat_num", -1)
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        if getattr(self, 'mm_projector', None) is None or self.multi_frame_projector:
            if self.multi_frame_projector:
                if getattr(self, 'mm_projector', None) is not None and self.mf_split_init:
                    mf_projector = nn.Sequential(
                        nn.Linear(self.config.mm_hidden_size * self.multi_frame_num,
                                  self.config.hidden_size * self.multi_frame_num),
                        nn.GELU(),
                    ...
                    for i in range(self.multi_frame_num):
                        mf_projector[0].weight.data[i * self.config.hidden_size: (i + 1) * self.config.hidden_size,
                        i * self.config.mm_hidden_size: (i + 1) * self.config.mm_hidden_size] = mm_projector[
                            0].weight.data
                        mf_projector[0].bias.data[i * self.config.hidden_size: (i + 1) * self.config.hidden_size] = \
                        mm_projector[0].bias.data  # / self.multi_frame_num

                    for i in range(self.multi_frame_num):
                        mf_projector[2].weight.data[:, i * self.config.hidden_size: (i + 1) * self.config.hidden_size] = \
                        mm_projector[2].weight.data / self.multi_frame_num
...
class F16ForCausalLM(Qwen2ForCausalLM, LlavaMetaForCausalLM):
    config_class = F16Config
    ...
    def __init__(self, config, **audio_config):
        ...
        config.model_type = "f16"
        config.rope_scaling = None

        self.add_time_token = config.add_time_token

        self.model = F16Model(config)
    ...
    def encode_images(self, images, video_idx_in_batch=[], split_sizes=None):
        if self.get_model().multi_frame_projector:
            step = 100
            image_features = []
            ...
            if self.get_model().frame_repeat_num > 1:
                image_features = image_features.unsqueeze(1)
                image_features = image_features.repeat(1, self.get_model().frame_repeat_num, 1, 1)
                image_features = image_features.view(-1, image_features.size(-2), image_features.size(-1))

            tail_mf_num = self.get_model().multi_frame_num - image_features.size(0) % self.get_model().multi_frame_num
            if tail_mf_num > 0:
                image_features_tail = torch.zeros([tail_mf_num, image_features.size(1), image_features.size(2)],
                                                  device=image_features.device, dtype=image_features.dtype)
                image_features = torch.cat([image_features, image_features_tail], dim=0)

            image_features = image_features.reshape(-1, self.get_model().multi_frame_num, image_features.size(-2),
            ...
            if self.config.mm_pooling_position == "before":
                img_feat = self.get_2dPool(
                    img_feat)  # (num_vid*num_frames, 576, 4096) -> (num_vid*num_frames, 144, 4096)

            if img_feat.size(0) <= 110:
                img_feat = self.get_model().mm_projector(img_feat)  # (dim_1_sum, 576, 1024) -> (dim_1_sum, 576, 4096)
            else:
                tmp_img_feat = []
                step = 96
                for i in range(0, img_feat.size(0), step):
                    seg_img_feat = self.get_model().mm_projector(img_feat[i: i + step])
                    tmp_img_feat.append(seg_img_feat)

                img_feat = torch.cat(tmp_img_feat, dim=0)
    ...
    def prepare_inputs_labels_for_multimodal(
            ...
            images, modalities, real_time=None,
    ):
        vision_tower = self.get_vision_tower()
        if vision_tower is None or images is None or images[0] is None or (
                not isinstance(input_ids, list) and input_ids.shape[1] == 1):
            if images is not None and images[0] is None:
                if torch.cuda.current_device() == 0:
                    print(f'[RANK0 PRINT] | input_id\'s shape: {input_ids.shape}')
                        ...
                        if image_idx in video_idx_in_batch:
                            if self.config.mm_newline_position == "grid":  # here
                                ...
                                if self.add_time_token:
                                    ...
                                    time_idx = [num2time(round(frame_per_time * f_idx)) for f_idx in
                                                range(1, num_frames + 1)]
                                    time_tokens = [
                                        self.tokenizer(t_idx, return_tensors='pt')["input_ids"].to(self.device) for
                                        t_idx in time_idx]
                                    time_embeds = [self.get_model().embed_tokens(t_tok).squeeze() for t_tok in
                                                   time_tokens]
        ...
        if modality_max_length is None or modality_max_length == "None":
            ...
            modality_max_length_dict = {"image": modality_max_length[0], "text": modality_max_length[1],
                                        "video": modality_max_length[2]}
            new_input_embeds = [x[: modality_max_length_dict[modality]] for x, modality in
                                zip(new_input_embeds, modalities)]
    ...
    def forward(
        ...
        if dpo_forward:
            (input_ids, position_ids_v1, attention_mask, past_key_values, inputs_embeds,
            ...
            (reject_input_ids, position_ids_v2, reject_attention_mask, reject_past_key_values, reject_inputs_embeds,
             reject_labels) = self.prepare_inputs_labels_for_multimodal(reject_input_ids, position_ids,
                                                                        reject_attention_mask, past_key_values,
                                                                        reject_labels, images, modalities,
                                                                        real_time=real_time)
            ...
            if inputs_embeds is None:
                (input_ids, position_ids, attention_mask, past_key_values, inputs_embeds,
                 labels) = self.prepare_inputs_labels_for_multimodal(gt_input_ids, position_ids, gt_attention_mask,
                                                                     past_key_values, gt_labels, images, modalities,
                                                                     real_time=real_time)
    ...
    def generate(
        ...
        if images is not None and images[0] is not None:
            (inputs, position_ids, attention_mask, _, inputs_embeds, _) = self.prepare_inputs_labels_for_multimodal(
                inputs, position_ids, attention_mask, None, None, images, modalities, real_time=real_time)
        else:
            if torch.cuda.current_device() == 0:
                print(f'[RANK0 PRINT] | input_id\'s shape: {inputs.shape}')
...
AutoConfig.register("f16", F16Config)
AutoModelForCausalLM.register(F16Config, F16ForCausalLM)
