    ...
    def forward(
        ...
        if attention_mask is not None:
            if attention_mask.size() != (batch_size, 1, q_len, k_v_seq_len):
                raise ValueError(
                    f"Attention mask should size {(batch_size, 1, q_len, k_v_seq_len)}, but is {attention_mask.size()}"
                )
...
class SiglipFlashAttention2(SigLipAttention):
    """
    SiglipAttention flash attention module.
    This module inherits from `SiglipAttention` as the weights of the module stays untouched.
...
class SigLipVisionTowerFlash(nn.Module):
    def __init__(self, vision_tower, vision_tower_cfg, delay_load=False):
        ...
        if not delay_load:
            ...
            rank0_print("The checkpoint contain `vision_tower` weights: `mm_tunable_parts` contains `mm_vision_tower`.")
            self.load_model()
        else:
