...
class LLaVADPOTrainer(Trainer):
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        ...
        print(f"Save model at {os.path.join(output_dir, f'checkpoint-{self.state.global_step}.bin')}")
    ...
    def _inner_training_loop(
        ...
        chosen_rewards = torch.tensor(0.0).to(args.device)
        rejected_rewards = torch.tensor(0.0).to(args.device)
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):                                                                                                    
                ...
                with self.accelerator.accumulate(model):
                    tr_loss_step, chosen_rewards_step, rejected_rewards_step = self.training_step(model, inputs)
                ...
                chosen_rewards += chosen_rewards_step
                rejected_rewards += rejected_rewards_step
                ...
                if (
                    ...
                    self._maybe_log_save_evaluate(tr_loss, grad_norm, model, trial, epoch, ignore_keys_for_eval, chosen_rewards, rejected_rewards)
                else:
    ...
    def _maybe_log_save_evaluate(self, tr_loss, grad_norm, model, trial, epoch, ignore_keys_for_eval, chosen_rewards=None, rejected_rewards=None):
        if self.control.should_log and self.state.global_step > self._globalstep_last_logged:
            ...
            if chosen_rewards is not None:
                logs["chosen_rewards"] = chosen_rewards.detach().item() if isinstance(chosen_rewards, torch.Tensor) else chosen_rewards
                chosen_rewards -= chosen_rewards
            if rejected_rewards is not None:
                logs["rejected_rewards"] = rejected_rewards.detach().item() if isinstance(rejected_rewards, torch.Tensor) else rejected_rewards
    ...
    def prediction_step(self, model, inputs, prediction_loss_only=False, ignore_keys=None, do_sample=False):
        if True:
            ...
            if prediction_loss_only:
                ...
                return ([outputs.loss.item()], [None], [None])
            
            ids = inputs.pop("ids", None)
            prompts = inputs.pop("prompts", None)
            labels = inputs.pop("labels", None)
            texts = inputs.pop("texts", None)
            ce_only = inputs.pop("ce_only", None)
            _ = inputs.pop("ori_item", None)
            ...
            with torch.no_grad():
                ...
                preds1 = [self.tokenizer.decode(t) for t in generated_tokens]
                ...
                preds2 = [self.tokenizer.decode(t) for t in generated_tokens_2]

            return (None, preds1, preds2, texts)
    ...
    def prediction_loop(self, dataloader):
        ...
        if torch.cuda.current_device() == 0:
            for inputs in tqdm(dataloader):
                ...
                loss, preds1, preds2, refs = self.prediction_step(model, inputs)
                prompts = inputs['prompts']
                results.append((ids, prompts, preds1, preds2, refs))
        else:
        ...
        for batch_item in results:
            for i in range(len(batch_item[0])):
                id, prp, pred1, pred2, ref = batch_item[0][i], batch_item[1][i], batch_item[2][i], batch_item[3][i], batch_item[4][i]
                output_data.append({
                    "id": id,
                    'prompt': prp,
                    "pred1": pred1,
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        with self.compute_loss_context_manager():
            loss, chosen_rewards, rejected_rewards = self.compute_loss(model, inputs)
        ...
        return loss.detach() / self.args.gradient_accumulation_steps, chosen_rewards.detach() / self.args.gradient_accumulation_steps, rejected_rewards.detach() / self.args.gradient_accumulation_steps
    ...
    def dpo_loss(
        ...
        if dist.get_rank() == 0:
            print(f"chosen: {policy_chosen_logps.item()}, rejected: {policy_rejected_logps.item()}, ref chosen: {reference_chosen_logps.item()}, ref rejected: {reference_rejected_logps.item()}")
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        if "ce_only" in inputs and inputs["ce_only"][0]:
            inputs.pop("ce_only")
            inputs["dpo_forward"] = False
            ce_outputs = model(**inputs)
            ce_loss = ce_outputs["loss"] if isinstance(ce_outputs, dict) else ce_outputs[0]
            print(f">>> [RANK {dist.get_rank()} PRINT] | ce_loss_only: {ce_loss.item()}")
            loss = ce_loss
            chosen_rewards, rejected_rewards = torch.tensor(0.0, dtype=loss.dtype, device=loss.device), torch.tensor(0.0, dtype=loss.dtype, device=loss.device)

        else:
            logits, labels, reject_logits, reject_labels, vocab_size = model(**inputs)
            labels = labels[:, -inputs['labels'].shape[-1]:]
            reject_labels = reject_labels[:, -inputs['reject_labels'].shape[-1]:]
            logp = self.get_batch_logps(logits, labels)
            rej_logp = self.get_batch_logps(reject_logits, reject_labels)
            with torch.no_grad():
                with self.null_ref_context():
                    ref_logits, _, ref_rej_logits, _, _ = self.model(**inputs)

            ref_logp = self.get_batch_logps(ref_logits, labels)
            ref_rej_logp = self.get_batch_logps(ref_rej_logits, reject_labels)

            loss, chosen_rewards, rejected_rewards = self.dpo_loss(logp, rej_logp, ref_logp, ref_rej_logp)
            loss = loss.mean()
            chosen_rewards = chosen_rewards.mean()
            rejected_rewards = rejected_rewards.mean()

            if self.args.with_ce_loss:
                inputs["dpo_forward"] = False
                ce_outputs = model(**inputs)
                ce_loss = ce_outputs["loss"] if isinstance(ce_outputs, dict) else ce_outputs[0]
                print(f">>> [RANK {dist.get_rank()} PRINT] | ce_loss: {ce_loss.item()}, dpo_loss: {loss.item()}, chosen_rewards: {chosen_rewards.item()}, rejected_rewards: {rejected_rewards.item()}")
                loss = loss + self.args.ce_loss_weight * ce_loss

        return (loss, logits, chosen_rewards, rejected_rewards) if return_outputs else loss, chosen_rewards, rejected_rewards
