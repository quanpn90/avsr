...
def train():
    ...
    if not training_args.load_from_lora:
        ...
        model = F16ForCausalLM.from_pretrained(
            training_args.ckpt,
        ...
        model, tokenizer = load_qwen_lora_model(training_args.ckpt, model_base=training_args.model_base, lora_enable=training_args.lora_enable, load_full=training_args.load_full, lora_r=training_args.lora_r, lora_alpha=training_args.lora_alpha, lora_dropout=training_args.lora_dropout, model_max_length=training_args.model_max_length, new_model_args=model_args)
        # if True:
