...
class FANPredictor(object):
    ...
    def get_model(name: str = '2dfan2') -> SimpleNamespace:
        name = name.lower()
        if name == '2dfan2':
            return SimpleNamespace(weights=os.path.join(os.path.dirname(__file__), '..','..','..','..','model-bin','face_alignment','fan','weights', '2dfan2.pth'),
            ...
            return SimpleNamespace(weights=os.path.join(os.path.dirname(__file__), '..','..','..','..','model-bin','face_alignment','fan','weights', '2dfan4.pth'),
            ...
            return SimpleNamespace(weights=os.path.join(os.path.dirname(__file__), '..','..','..','..','model-bin','face_alignment','fan','weights', '2dfan2_alt.pth'),
                                   config=SimpleNamespace(crop_ratio=0.55, input_size=256, num_modules=2,
                                                          hg_num_features=256, hg_depth=4, use_avg_pool=False,
                                                          use_instance_norm=False, stem_conv_kernel_size=7,
                                                          stem_conv_stride=2, stem_pool_kernel_size=2,
